...
| 版本       | 日期         | 作者  | 变更说明                                  |
| -------- | ---------- | --- | ------------------------------------- |
| **v3.4** | 2026-04-03 | CRT | **计划-台账深度集成**：功能模块独立实体化、生命周期日志、双向追溯、状态同步机制 |
| **v3.3** | 2026-03-27 | CRT | **计划类型重构与SOP模板体系化**                   |
| **v3.1** | 2026-03-26 | CRT | **全局架构一致性修正**：添加用户故事对应关系注释，明确实体与故事的映射 |
| v3.0     | 2026-03-26 | CRT | **重构模块划分**：• 扩展为8个模块，补充完整的实体定义和模块交互图  |
...
```
┌─────────────────────────────────────────────────────────────┐
│ 第一层：计划层 (Plan) - 甲方运维经理（需求方）视角（v3.4更新）        │
│  ├─ 计划创建（MGR-01）：创建上线/变更计划                          │
│  ├─ 台账范围选择（MGR-02）：5种分类对应不同操作                      │
│  │   • 新系统上线 → 调用"创建应用系统"接口，创建新台账+功能模块       │
│  │   • 新功能上线 → 调用"选择应用系统"接口，单选后添加功能模块        │
│  │   • 功能变更   → 调用"选择应用系统"接口，多选后选择功能模块        │
│  │   • 架构变更   → 调用"选择应用系统"接口，多选后选择功能模块+安全检查 │
│  │   • 安全检查   → 调用"选择应用系统"接口，面向应用系统层级          │
│  ├─ 工作项自动生成（MGR-03）：根据分类自动推荐初始工作项               │
│  └─ 计划-台账双向追溯：计划详情展示关联台账的生命周期时间线            │
└─────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────┐
...
└─────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────┐
│ 第三层：台账层 (Inventory) - 基础数据（v3.4更新）                  │
│  ├─ 应用系统台账：应用名称/系统类型/部署环境/负责人/状态            │
│  ├─ 功能模块台账（独立实体）：模块编码/名称/负责人/版本/状态        │
│  │   └─ 支持层级结构、版本管理、变更追溯                         │
│  ├─ 云服务台账：IAAS资源/PAAS软件/关联应用系统                    │
│  ├─ 账号台账：系统账户/软件账户/权限级别/有效期                   │
│  └─ 生命周期日志：记录系统上线、变更、下线等关键事件               │
└─────────────────────────────────────────────────────────────┘
...
```
┌───────────────┐      ┌─────────────────┐      ┌──────────────────────────────────────┐
│   计划类型     │  →   │ 工作流模板类型   │  →   │          标准化检查项清单              │
├───────────────┤      ├─────────────────┤      ├──────────────────────────────────────┤
│ 新系统上线     │  →   │ 新系统上线模版   │  →   │ • 基础资源标准化交付                  │
│               │      │                 │      │ • 服务对象台账更新                    │
│               │      │                 │      │ • 上线前安全检测                      │
│               │      │                 │      │ • 监控告警配置确认                    │
├───────────────┤      ├─────────────────┤      ├──────────────────────────────────────┤
│ 新功能上线     │  →   │ 新功能上线模版   │  →   │ （默认继承关联系统的检查项配置）        │
│               │      │                 │      │ • 服务对象台账更新（功能模块）          │
│               │      │                 │      │ • 监控告警配置确认                    │
├───────────────┤      ├─────────────────┤      ├──────────────────────────────────────┤
│ 功能变更       │  →   │ 功能变更模版     │  →   │ • 服务对象台账更新                    │
│               │      │                 │      │ • 监控告警配置确认                    │
├───────────────┤      ├─────────────────┤      ├──────────────────────────────────────┤
│ 架构变更       │  →   │ 架构变更模版     │  →   │ • 基础资源标准化交付                  │
│               │      │                 │      │ • 服务对象台账更新                    │
│               │      │                 │      │ • 上线前安全检测                      │
│               │      │                 │      │ • 监控告警配置确认                    │
├───────────────┤      ├─────────────────┤      ├──────────────────────────────────────┤
...
#### 2.2.2 各模板类型的标准化父工作项（检查项）

| 工作流模板类型     | 父工作项（检查项）名称  | 说明                       | 适用场景      |
| ----------- | ------------ | ------------------------ | --------- |
| **新系统上线模版** | 1. 基础资源标准化交付 | 云服务器/数据库/中间件等资源的标准化配置    | 新系统首次上线   |
| <br />      | 2. 服务对象台账更新  | 应用系统台账、云服务台账、账号台账的建立     | <br />    |
| <br />      | 3. 上线前安全检测   | 漏洞扫描、基线检查、渗透测试           | <br />    |
| <br />      | 4. 监控告警配置确认  | 监控项配置、告警规则设置、通知渠道验证      | <br />    |
| **新功能上线模版** | 1. 服务对象台账更新  | 功能模块更新、关联配置变更            | 现有系统新增功能  |
| <br />      | 2. 监控告警配置确认  | 新增功能的监控覆盖确认              | <br />    |
| **功能变更模版**  | 1. 服务对象台账更新  | 功能配置变更、参数调整              | 现有功能修改/优化 |
| <br />      | 2. 监控告警配置确认  | 变更后的监控有效性确认              | <br />    |
| **架构变更模版**  | 1. 基础资源标准化交付 | 架构调整涉及的资源变更（扩容/迁移/数据库变更） | 系统架构调整    |
| <br />      | 2. 服务对象台账更新  | 架构变更后的台账同步更新             | <br />    |
| <br />      | 3. 上线前安全检测   | 架构变更后的安全验证               | <br />    |
| <br />      | 4. 监控告警配置确认  | 架构变更后的监控配置调整             | <br />    |
| **安全检查模版**  | 1. 系统漏洞修复    | 漏洞扫描与修复验证                | 定期/专项安全检查 |
| <br />      | 2. 基线加固      | 安全基线配置检查与加固              | <br />    |
| <br />      | 3. 渗透测试      | 渗透测试执行与问题修复              | <br />    |
| <br />      | 4. 文件安全      | 敏感文件扫描、权限检查              | <br />    |

#### 2.2.3 台账操作规则（v3.4更新）

| 计划分类 | 台账操作方式 | 关联台账类型 | 功能模块操作 | 日志类型 | 特殊说明 |
|---------|-------------|-------------|-------------|---------|---------|
| **新系统上线** | **新增**应用系统台账 | 应用系统、云服务、账号 | 创建时添加功能模块 | `system_launch` | 调用"创建应用系统"接口，事务级联创建 |
| **新功能上线** | **查询**已有台账 | 应用系统台账 | **新增**功能模块 | `module_launch` | 调用"选择应用系统"接口，单选系统后添加模块 |
| **功能变更** | **查询+勾选**已有台账 | 应用系统台账 | **选择**已有功能模块 | `module_update` | 调用"选择应用系统"接口，多选系统后选择模块 |
| **架构变更** | **查询+勾选**已有台账 | 应用系统、云服务 | **选择**已有功能模块 | `system_upgrade` | 调用"选择应用系统"接口，可能涉及云资源变更 |
| **安全检查** | **查询**已有台账 | 应用系统台账（可选范围） | 不涉及 | `security_check` | 面向应用系统层级，全系统或指定范围扫描 |

**功能模块独立实体说明**（v3.4新增）：
- 功能模块已从JSON字段重构为独立实体表 `inventory_function_modules`
- 支持版本管理、状态流转、层级结构、变更追溯
- 模块状态：draft → developing → testing → online → offline
- 每个模块变更自动记录到生命周期日志

**数据同步机制**：
1. **实时同步**：创建/更新计划时同步调用台账管理接口
2. **状态同步**：计划状态变更（DRAFT→IN_PROGRESS→COMPLETED）触发台账状态同步
3. **双向追溯**：计划详情展示关联台账的生命周期时间线

> **注意**：子工作项（验收项/检查项）在父工作项下定义，具体内容根据业务场景从知识库提取或自定义配置。

<br />

#### 2.2.4 计划-台账集成架构（v3.4新增）
...
|---------|---------|---------|---------|
| DRAFT | - | 创建计划 | - |
| IN_PROGRESS | active | 启动计划 | 根据分类生成对应日志 |
| COMPLETED | active | 完成计划 | 记录完成事件 |
| CANCELLED | active/评估 | 取消计划 | 根据业务规则评估是否回滚 |

**生命周期日志类型**：

| 日志类型 | 触发场景 | 关联计划分类 |
|---------|---------|-------------|
| `system_launch` | 新系统首次上线 | 新系统上线 |
| `module_launch` | 功能模块首次上线 | 新功能上线 |
| `module_update` | 功能模块更新 | 功能变更 |
| `system_upgrade` | 系统版本升级 | 架构变更 |
| `security_check` | 安全检查执行 | 安全检查 |
...
| MGR-02 | **作为**甲方运维经理（需求方），**我需要**根据计划分类（新系统上线/新功能上线/功能变更/架构变更/安全检查）选择不同的台账操作方式，**以便**准确关联涉及的系统范围 | 支持5种不同的台账操作场景，安全检查无需关联台账 | 计划管理、台账管理     |
| MGR-03 | **作为**甲方运维经理（需求方），**我需要**系统根据计划类型自动匹配工作流模板类型并生成标准化的父工作项（检查项），**以便**快速搭建工作流框架             | 基于5种模板类型自动生成标准检查项，减少人工配置 | 计划管理、SOP模板引擎  |
...
| **Module\_03** | SOP Template Engine     | 管理SOP模板（5种标准模板类型）、审核矩阵、强制规则，驱动工作项自动生成 | P1  |
...
核心实体:
  - Plan:              # 计划 - MGR-01
      - plan_id (PK), name, 
        category: enum   # 计划分类 [new_system/new_feature/func_change/arch_change/security_check] - MGR-02
        template_type: enum  # 工作流模板类型 [new_system/new_feature/func_change/arch_change/security] - MGR-03
...
核心实体:
  - Application:       # 应用系统台账
      ...
      # - new_feature: 编辑功能模块
      # - func_change: 查询并勾选
      # - arch_change: 查询并勾选（可能涉及云资源/数据库变更）
      # - security_check: 不关联台账
      
  - CloudResource:     # 云服务资源台账
      - id (PK), app_id (FK), resource_type, resource_name, configuration (JSON)
      # arch_change类型计划根据是否涉及云资源决定是否关联云服务台账
...
核心实体:
  - SOPTemplate:       # SOP模板 - MGR-04
      - template_id (PK), template_name, version, status
      - template_type: enum  # 模板类型 [new_system/new_feature/func_change/arch_change/security]
      - audit_matrix_config_id (FK)  # 关联审核矩阵 - MGR-07
      - parent_work_items_config (JSON)  # 父工作项（检查项）配置清单，按模板类型预定义
