...
> **更新日期**: 2026-03-27
> 
> **变更说明**: 同步全局架构 v3.3，更新计划类型与台账操作映射关系（5种计划类型）
...
| 计划分类 | 台账操作 | 关联台账类型 | 说明 |
|---------|---------|-------------|------|
| **新系统上线** | **新增**应用系统台账 | 应用系统、云服务、账号 | 创建全新的台账条目，事务级联创建 |
| **新功能上线** | **编辑**已有台账（功能模块） | 应用系统台账 | 单选台账，编辑功能模块字段 |
| **功能变更** | **查询+勾选**已有台账 | 应用系统台账 | 多选应用系统，标记受影响范围 |
| **架构变更** | **查询+勾选**已有台账 | 应用系统、云服务 | 多选应用系统，可能涉及云资源/数据库变更 |
...
- 支持标签继承（子系统继承父系统标签）
- **安全检查类型计划**：若选择指定范围，则对范围内的资源打标签；若选择全系统，则对所有活跃资源生效
...
台账管理模块为计划管理提供基础数据支撑，不同计划类型的交互方式不同：
...
**关键设计**：
- 新系统上线时，台账创建与计划创建在同一个事务中
- 避免产生"孤儿台账"（已创建台账但计划创建失败）
- 支持幂等性，通过 `idempotency_key` 防止重复创建
...
**关键设计**：
- 仅记录 `related_plan_ids` 关联
- 架构变更可能涉及云资源变更，需同步更新云服务台账
...
**关键设计**：
- 全系统扫描：不关联具体台账，对所有活跃资源生效
- 指定范围：对选中的台账打标签，核验时仅检查带标签资源

### 7.2 台账查询接口（供计划模块调用）

| 接口 | 用途 | 调用方 |
|-----|------|-------|
| `GET /api/v1/inventory/applications?status=active` | 查询可关联的台账列表 | Module_01 |
| `GET /api/v1/inventory/applications/:id` | 获取台账详情 | Module_01, Module_03 |
| `POST /api/v1/inventory/applications` | 创建新台账（新系统上线） | Module_01 |
| `PUT /api/v1/inventory/applications/:id/function-modules` | 更新功能模块（新功能上线） | Module_01 |
| `POST /api/v1/inventory/applications/:id/tag` | 打标签（安全检查指定范围） | Module_01 |
