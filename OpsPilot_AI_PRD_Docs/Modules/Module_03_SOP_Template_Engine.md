...
|-----|------|------|------|
| 模板名称 | String | 是 | 模板标题 |
| 模板类型 | Enum | 是 | new_system/new_feature/func_change/arch_change/security |
| 模板描述 | Text | 否 | 模板说明 |
| 版本 | String | 是 | 模板版本号 |
...
根据全局架构定义，5种计划类型对应5种工作流模板类型：

| 计划分类 | 模板类型 | 默认模板名称 | 说明 |
|---------|---------|-------------|------|
| 新系统上线 | new_system | 新系统上线检查流程 | 包含完整的4大检查项 |
| 新功能上线 | new_feature | 新功能上线检查流程 | 侧重功能模块相关检查 |
| 功能变更 | func_change | 功能变更检查流程 | 侧重功能配置变更检查 |
| 架构变更 | arch_change | 架构变更检查流程 | 包含架构调整相关检查（含数据库变更） |
| 安全检查 | security_check | 安全检查流程 | 专项安全检查流程 |

#### 2.1.3 标准化父工作项（检查项）配置
...
| 序号 | 父工作项名称 | 说明 | 必需 |
|-----|------------|------|-----|
| 1 | 基础资源标准化交付 | 云服务器/数据库/中间件等资源的标准化配置 | 是 |
| 2 | 服务对象台账更新 | 应用系统台账、云服务台账、账号台账的建立 | 是 |
| 3 | 上线前安全检测 | 漏洞扫描、基线检查、渗透测试 | 是 |
| 4 | 监控告警配置确认 | 监控项配置、告警规则设置、通知渠道验证 | 是 |

**新功能上线模版 (new_feature)**：
| 序号 | 父工作项名称 | 说明 | 必需 |
|-----|------------|------|-----|
| 1 | 服务对象台账更新 | 功能模块更新、关联配置变更 | 是 |
| 2 | 监控告警配置确认 | 新增功能的监控覆盖确认 | 是 |

**功能变更模版 (func_change)**：
| 序号 | 父工作项名称 | 说明 | 必需 |
|-----|------------|------|-----|
| 1 | 服务对象台账更新 | 功能配置变更、参数调整 | 是 |
| 2 | 监控告警配置确认 | 变更后的监控有效性确认 | 是 |

**架构变更模版 (arch_change)**：
| 序号 | 父工作项名称 | 说明 | 必需 |
|-----|------------|------|-----|
| 1 | 基础资源标准化交付 | 架构调整涉及的资源变更（扩容/迁移/数据库变更） | 是 |
| 2 | 服务对象台账更新 | 架构变更后的台账同步更新 | 是 |
| 3 | 上线前安全检测 | 架构变更后的安全验证 | 是 |
| 4 | 监控告警配置确认 | 架构变更后的监控配置调整 | 是 |

**安全检查模版 (security)**：
| 序号 | 父工作项名称 | 说明 | 必需 |
|-----|------------|------|-----|
| 1 | 系统漏洞修复 | 漏洞扫描与修复验证 | 是 |
| 2 | 基线加固 | 安全基线配置检查与加固 | 是 |
| 3 | 渗透测试 | 渗透测试执行与问题修复 | 是 |
| 4 | 文件安全 | 敏感文件扫描、权限检查 | 是 |
...
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  1. 用户创建计划，选择分类(category)                              │
│           ↓                                                      │
│  2. 系统根据分类自动匹配 template_type                            │
│           ↓                                                      │
│  3. 查询该 template_type 的 active 模板                           │
│           ↓                                                      │
│  4. 获取模板的父工作项（检查项）配置                               │
│           ↓                                                      │
│  5. 根据台账范围，生成具体的工作项实例                             │
...
| 计划分类 | 匹配模板类型 | 生成检查项 | 台账操作 |
|---------|------------|-----------|---------|
| 新系统上线 | new_system | 4项：基础资源/台账/安全/监控 | 新增台账 |
| 新功能上线 | new_feature | 2项：台账(功能模块)/监控 | 编辑台账(功能模块) |
| 功能变更 | func_change | 2项：台账/监控 | 勾选台账 |
| 架构变更 | arch_change | 4项：基础资源/台账/安全/监控 | 勾选台账(含云资源) |
| 安全检查 | security | 4项：漏洞/基线/渗透/文件安全 | 不关联台账 |
...
| `{app_name}` | 应用名称 | 台账数据 |
| `{plan_id}` | 计划ID | 计划数据 |
| `{current_date}` | 当前日期 | 系统时间 |
| `{business_owner}` | 业务负责人 | 台账数据 |
| `{project_owner}` | 项目负责人 | 台账数据 |

**使用示例**：
- 工作项名称："`{app_name}` 基础资源标准化交付"
- 生成后："订单管理系统 基础资源标准化交付"
...
| template_type | ENUM('new_system','new_feature','func_change','arch_change','security') | NOT NULL | 是 | 模板类型（对应5种计划类型） |
| category | VARCHAR(50) | - | 否 | 保留字段（兼容旧版本） |
...
| parent_work_items_config | JSON | - | 否 | 父工作项（检查项）配置清单，按模板类型预定义 |
...
**Rule 1: 模板类型与计划类型的强绑定**
- 每种模板类型 (`template_type`) 对应一种计划分类 (`plan.category`)
- 创建计划时，系统根据 `plan.category` 自动匹配对应的 `template_type`
- 一个模板类型可以有多个版本的模板，但同一时间只能有一个 `active` 版本

**Rule 2: 标准化检查项继承**
- 基于模板类型创建模板时，系统自动预填充该类型对应的标准化父工作项（检查项）
- 预填充的检查项可编辑、可删除、可添加新的检查项
- 子工作项（验收项）从知识库提取或手动配置
...
**触发时机**：
- 计划创建完成（Step 3提交后）
- 计划状态从 `DRAFT` → `PENDING` 或 `IN_PROGRESS` 时
...
```json
{
  ...
  "inventory_scope": {
    ...
    "cloud_resource_ids": ["res-001", "res-002"],
    "account_ids": ["acc-001"]
  }
}
```
...
**实例化流程**：
1. 验证模板状态为 `active`
2. 验证 `plan.category` 与 `template.template_type` 匹配
3. 获取模板的所有流程节点和父工作项（检查项）
4. 为每个检查项创建工作项实例（`WorkItemInstance`）
5. 为每个检查项从知识库提取或复制子工作项模板（验收项）
6. 执行变量替换
7. 创建工作流实例（`WorkflowInstance`）
8. 关联到计划，触发通知

**响应**：
```json
{
  "workflow_instance_id": "WF-20240326-001",
  "plan_id": "PLAN-20240326-001",
  "template_id": "SOP-NEW-001",
  "generated_work_items": [
    {
      "work_item_id": "WI-001",
      "name": "订单管理系统 基础资源标准化交付",
      "category": "base_resource",
      "audit_level": "critical",
      "sub_items": [
        {"sub_item_id": "SI-001", "name": "ECS实例配置检查"},
        {"sub_item_id": "SI-002", "name": "RDS实例配置检查"}
      ]
    }
  ],
...
```
Module_01(计划管理)                    Module_03(SOP模板引擎)
       │                                      │
       │  1. 用户选择计划分类(category)         │
       │─────────────────────────────────────>│
       │  2. 返回匹配的 template_type           │
       │<─────────────────────────────────────│
       │                                      │
       │  3. 用户确认模板（或使用默认）          │
       │─────────────────────────────────────>│
...
| 接口 | 用途 | 调用方 |
|-----|------|-------|
| `GET /api/sop-templates/by-type/{template_type}` | 获取指定类型的活跃模板 | Module_01 |
| `GET /api/sop-templates/{id}/parent-work-items` | 获取模板的父工作项配置 | Module_01 |
| `POST /api/sop-templates/{id}/instantiate` | 实例化模板为工作流 | Module_01 |

### 7.3 数据流转

```
SOP模板设计阶段：
SOPTemplate (parent_work_items_config JSON)
├── 基础资源标准化交付
├── 服务对象台账更新
├── 上线前安全检测
└── 监控告警配置确认
...
计划创建阶段：
Plan (category=new_system)
  ↓ 自动匹配
Template (template_type=new_system)
  ↓ 实例化
WorkItemInstance[] (父工作项实例)
├── WorkItemInstance (基础资源标准化交付)
│   └── SubWorkItemInstance[] (验收项/子工作项)
├── WorkItemInstance (服务对象台账更新)
│   └── SubWorkItemInstance[]
...
```
